  ...
  try {
    switch (action.action_type) {
      ...
      case "comment_story":
        ...
        const randomStoryComment =
          action.metadata?.content || storyComments[Math.floor(Math.random() * storyComments.length)];
        const { error: commentStoryError } = await supabase
          .from("story_comments")
          .insert({
            story_id: action.target_id,
            user_id: ghostUserId,
            content: randomStoryComment,
          });
        if (commentStoryError) throw commentStoryError;
        // Update story comments count
        await supabase.rpc("increment_story_comments_count", {
          p_story_id: action.target_id,
        });
        break;
      ...
      case "comment_post":
        ...
        const randomForumComment =
          action.metadata?.content || forumComments[Math.floor(Math.random() * forumComments.length)];
        const { error: commentPostError } = await supabase
          .from("forum_comments")
          .insert({
            post_id: action.target_id,
            user_id: ghostUserId,
            content: randomForumComment,
          });
        if (commentPostError) throw commentPostError;
        // Update forum post comments count
        const { data: currentForumPost } = await supabase
          .from("forum_posts")
        ...
        await supabase
          .from("forum_posts")
          .update({
            comments_count: (currentForumPost?.comments_count || 0) + 1,
          })
          .eq("id", action.target_id);
        break;

      case "comment_community_post":
        if (!action.target_id) throw new Error("Target ID required");
        ...
        const randomCommunityComment =
          action.metadata?.content || communityComments[Math.floor(Math.random() * communityComments.length)];
        const { error: commentCommunityError } = await supabase
          .from("community_post_comments")
          .insert({
            post_id: action.target_id,
            user_id: ghostUserId,
            content: randomCommunityComment,
          });
        if (commentCommunityError) throw commentCommunityError;
        // Update community post comments count
        const { data: currentCommunityPost } = await supabase
          .from("community_posts")
          .select("comments_count")
          .eq("id", action.target_id)
          .single();
        ...
        await supabase
          .from("community_posts")
          .update({
            comments_count: (currentCommunityPost?.comments_count || 0) + 1,
          })
          .eq("id", action.target_id);
        break;
